...
with open(budget_data_csv_path, newline="") as csvfile:
    ...
    for row in csv_reader:
        ...
        if (count_date == 1):
            ...
            profit_loss_change = current_date_profit_loss - previous_date_profit_loss

            # Append each date to the dates[]
            date.append(row[0])

            # Append each profit_loss_change to the profit_loss_changes[]
            profit_loss_changes.append(profit_loss_change)

            # Make the current_date_loss to be previous_date_profit_loss for the next loop
            previous_date_profit_loss = current_date_profit_loss
...
budget_file = os.path.join("..", "Solution", "budget_results.text")
